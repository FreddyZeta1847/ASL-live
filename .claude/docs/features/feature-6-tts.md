...
5. **Failure handling.**
   - Missing voice file → `Speaker.__init__` raises `FileNotFoundError`.
     **No fallback.** Earlier draft had eSpeak-NG fall back per missing
     language, dropped during phase 5 planning for three reasons:
     1. eSpeak-NG is a system binary, not a pip package — adds an
        OS-level install step (`apt install espeak-ng` on Pi,
        `winget install eSpeak-NG` on Windows) that's easy to forget
        and breaks the "one `pip install -e .[pi]` and you're done"
        invariant we hold for the rest of the stack.
     2. The quality gap is enormous. Piper is neural; eSpeak-NG is
        formant synthesis from the 1990s. A silent fallback would
        mask a setup bug behind a sudden, jarring quality cliff that
        the user has no way to diagnose without reading the logs.
     3. The only realistic cause of a missing voice is "the setup
        script didn't run to completion." That's a setup bug, not a
        runtime condition. Hard failure surfaces the bug at boot
        instead of mid-conversation.
     Revisit if Piper ever drops support for one of our 5 languages
     and there is no replacement neural option.
