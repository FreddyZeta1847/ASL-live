...
This feature is documented in more depth than the others because the
...
- **RNN / LSTM / GRU** is for sequences over time. We classify single
  frames; there's no temporal sequence at this stage. (Feature 4
...
- **No rotation augmentation.** Feature 1 decided that rotation matters
  for distinguishing some sign pairs; augmenting it would teach the
  model to ignore rotation, breaking those distinctions.
- **No flip / mirror augmentation.** Mirror flips happen at data ingest
  time (feature 2). Doing it again at training time would double-count,
  doubling the model's belief that left and right hands are
  interchangeable (which we already established).
...
**How the numbers are computed (worked example).**

Precision, recall, and F1 are all computed *per class*, and all three
can be read directly off the confusion matrix. Pick a class — say
SPACE — and look at its **row** (actuals) and **column** (predictions):
...
Actual B       2  595  ...    0      1
...
Actual SPACE   3   0   ...  485      2      ← row total: 500
Actual DEL     7   0   ...    0    488
                              ↑
                              column total for SPACE: 1 + 0 + … + 485 + 0 = 489
```

For SPACE:

- **TP** (true positives — model said SPACE *and* it was SPACE)
  = `cm[SPACE][SPACE]` = **485**.
- **Row total** (every time SPACE actually occurred) = **500**.
- **Column total** (every time the model predicted SPACE) = **489**.

Plug in:

- **Recall_SPACE**    = TP / row_total    = 485 / 500 = **0.970**
- **Precision_SPACE** = TP / column_total = 485 / 489 = **0.992**
- **F1_SPACE**        = 2 × (0.992 × 0.970) / (0.992 + 0.970) = **0.981**

Repeat for all 26 classes → **26 per-class F1 scores**.

**Macro-F1 — unweighted average of per-class F1.**

Two-step computation:

1. Compute per-class F1 for each of the 26 classes (run the formula
   above on each row + column of the confusion matrix) → 26 numbers.
2. **Macro-F1 = simple unweighted mean of those 26 numbers.**

The "macro" qualifier matters: the alternative ("micro-F1") weights
every *sample* equally instead of every *class*, which brings back the
imbalance problem — 5,000 letter samples would drown out 1,000 SPACE
samples. Macro-F1 forces the model to be good at *every* class
regardless of frequency, which is exactly what this device's UX needs.

**Implementation notes (for `metrics.py`):**

- **Build the confusion matrix once**, then derive precision/recall/F1
  as row/column arithmetic. Don't iterate the prediction list 26 times.
- **Edge case — class with zero predictions.** If column_total = 0 for
  some class, precision is undefined (0/0). Convention: set F1 = 0 for
  that class (mirror sklearn's `zero_division=0` default). It pulls
  macro-F1 down — correctly, since "the model never predicts class X"
  is a real failure mode, not a free pass.
- **Edge case — class with zero actuals.** Shouldn't happen given the
  stratified split, but if it does, also F1 = 0 by the same convention.
- **Sanity-check against** `sklearn.metrics.f1_score(..., average='macro')`
  in unit tests with tolerance 1e-9.
...
2. **No off-diagonal cell exceeds 3 % of its row total** — for any
   actual class, no specific wrong prediction happens more than 3 % of
   the time.
...
**Solution.** The runtime debounce (feature 4) only counts a frame
