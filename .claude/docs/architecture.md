...
| Thumb-side (fist with thumb extended horizontally outward) | **DELETE** — remove last letter from buffer |

Selection rationale: both control gestures are visually distinct from any
letter shape so the landmark classifier won't confuse them. Thumb-side was
chosen over thumb-down for ergonomics: when the device is held in the
non-signing hand, rotating the wrist ~180° to point the thumb at the floor
is awkward, while extending the thumb sideways requires only ~90° rotation.
The gesture also reads as a backspace arrow (←), giving it intuitive
meaning for the user. The closest letter shape is "A" (closed fist with
thumb resting along the side of the fingers), but the fully extended thumb
places the thumb tip far enough from the wrist in normalized landmark
space that the MLP should separate them cleanly.
