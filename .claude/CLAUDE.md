...
4. **After every significant change** → commit + push (per the
   `git-autopush` skill).
5. **After any file-hierarchy change** → regenerate `tree.md` with the
   project script:
   ```powershell
   $env:PYTHONIOENCODING = "utf-8"; python .claude\generate_tree.py | Out-File -Encoding utf8 tree.md
   ```
   The `IGNORE` set inside the script controls what's excluded (`.claude`,
   `data/`, `models/`, `.venv/`, etc.) — extend it there, not by editing
   `tree.md` by hand.
